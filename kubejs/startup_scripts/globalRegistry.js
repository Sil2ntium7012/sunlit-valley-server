...
global.artisanGoods = [
  ...
  { item: "society:golden_mayonnaise", value: 1600 }, // [가격 조정] 6144 -> 1600
...
// ─── [가격 조정] 생선 원물 3배 ───────────────────────────────────────────────
// 훈제생선/알/숙성알은 아래 파생 루프가 "원래 생선 값"을 기준으로 계산하므로,
// 여기서 바로 곱하면 가공품 값까지 3배가 돼버립니다. 그래서 개수만 기억해뒀다가
// 파생 루프가 전부 끝난 뒤에 원물 구간에만 곱합니다.
const RAW_FISH_PRICE_MULTIPLIER = 3;
const rawFishCount = global.fish.length;
// ─────────────────────────────────────────────────────────────────────────────
// 54
global.smokedFish = [];
global.agedRoe = [];
...
// ─── [가격 조정] 생선 원물 3배 적용 ─────────────────────────────────────────
// rawFishCount 이후 인덱스는 위 루프가 새로 넣은 "알(roe)"이라 건드리지 않습니다.
// 훈제생선/숙성알은 이미 별도 배열로 계산이 끝나서 여기 영향을 안 받습니다.
...
// ─── [가격 조정] 절임류 너프 ────────────────────────────────────────────────
// "죄와 절임"(brine_and_punishment) 책이 +100%라 절임 루트가 지나치게 셌습니다.
//   1) 절인 아이템(= society:brine_and_punishment 태그에 든 21종) 판매가 1/2
//   2) 절이기 전 생채소 판매가도 1/2 (재료를 그냥 파는 우회로도 같이 조정)
// 두 조정은 서로 곱해지지 않습니다 - 절임 가격은 "절이기 전 원래 값"으로 이미
...
const PICKLED_ITEMS = new Set([
  ...
  "society:pickled_carrot",
  "society:pickled_pumpkin_slice",
  "society:pickled_lettuce",
  "society:pickled_eggplant",
  "society:pickled_potato",
  "society:pickled_golden_carrot",
  "society:pickled_ginger",
  "society:pickled_garlic",
  "society:pickled_bellpepper",
  "society:pickled_cauliflower",
  "society:pickled_zucchini",
  "society:pickled_turnip",
  "society:pickled_broccoli",
]);

const halvePrice = (entry) => {
  entry.value = Math.max(1, Math.round(entry.value / PICKLE_PRICE_DIVISOR));
};

// 1) 절인 아이템 - 절임은 요리(global.cooking)로 등록되지만, 혹시 다른 목록에
//    들어있는 게 있어도 놓치지 않도록 판매 목록 전체를 훑습니다.
[global.cooking, global.preserves, global.artisanGoods, global.animalProducts].forEach((list) => {
  list.forEach((entry) => {
    if (PICKLED_ITEMS.has(entry.item)) halvePrice(entry);
  });
});

// 2) 절이기 전 생채소 (절임 재료로 쓰이는 것들과 동일한 목록)
const PICKLABLE_RAW_ITEMS = new Set(global.picklableVegetables.map((v) => v.item));
global.crops.forEach((crop) => {
  if (PICKLABLE_RAW_ITEMS.has(crop.item)) halvePrice(crop);
});
// ─────────────────────────────────────────────────────────────────────────────
